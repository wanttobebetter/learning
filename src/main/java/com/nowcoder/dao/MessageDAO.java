...
public interface MessageDAO {
	...
	// Select *,count(id)as cnt from(SELECT* FROM message where from_id=12 ir
	// to_id=12 order by id desc) tt group by conversation_id order by id desc; tt
	...
	// group by conversation_id order by created_date desc limit #{offset},
	// #{limit}"})
